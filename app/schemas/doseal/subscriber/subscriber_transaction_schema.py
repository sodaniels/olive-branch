...
class SubscriberTransactionSchema(Schema):
    ...
    send_amount = fields.Float(
        required=True,
        error_messages={"required": "Send Amount is required", "invalid": "Invalid Send Amount"}
    )
...
class SubscriberTransactionExecuteSchema(Schema):
    checksum = fields.Str(
        required=True,
        validate=[
